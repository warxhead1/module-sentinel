...
export class ModernApiServer {
  ...
  async start(): Promise<void> {
    return new Promise((resolve, reject) => {
      this.server.listen(this.port, () => {
        console.log(`🚀 Module Sentinel API Server running on port ${this.port}`);
        resolve();
      });
   ...
   * Handle API requests
   */
  private async handleApiRequest(pathname: string, req: Request, res: Response, rawReq?: http.IncomingMessage) {
    // Emergency throttling to prevent spam
    const clientKey = String(req.headers['x-forwarded-for'] || req.headers['user-agent'] || 'unknown');
    const now = Date.now();
    ...
    if (clientData.count > this.MAX_REQUESTS_PER_SECOND) {
      console.warn(`Rate limiting client: ${clientKey} (${clientData.count} requests/sec)`);
    ...
    try {
      ...
      else if (apiPath.match(/^\/symbols\/file\/.+$/) && req.method === 'GET') {
        const qualifiedName = decodeURIComponent(apiPath.split('/').slice(3).join('/'));
        req.params.qualifiedName = qualifiedName;
        await this.symbolsRoutes.getFileSymbols(req, res);
      }
      else if (apiPath.match(/^\/symbols\/\d+\/relationships$/) && req.method === 'GET') {
      ...
      else if (apiPath.match(/^\/namespaces\/[^\/]+\/symbols$/) && req.method === 'GET') {
      ...
      else if (apiPath.match(/^\/projects\/\d+\/index$/) && req.method === 'POST') {
        const projectId = apiPath.split('/')[2];
        req.params.id = projectId;
        await this.handleIndexProject(req, res);
      }
      else if (apiPath.match(/^\/indexing\/jobs\/[\w-]+$/) && req.method === 'GET') {
      ...
      else if (apiPath.match(/^\/projects\/\d+\/detect-languages$/) && req.method === 'GET') {
        const projectId = apiPath.split('/')[2];
        req.params.id = projectId;
      ...
      else if (apiPath.match(/^\/projects\/\d+$/) && req.method === 'DELETE') {
        const projectId = apiPath.split('/')[2];
        req.params.id = projectId;
      ...
      else if (apiPath.match(/^\/code-flow\/call-graph\/\d+$/) && req.method === 'GET') {
        const symbolId = apiPath.split('/')[3];
        req.params.symbolId = symbolId;
        await this.codeFlowRoutes.getCallGraph(req, res);
      }
      else if (apiPath === '/code-flow/execution-paths' && req.method === 'GET') {
        await this.codeFlowRoutes.getExecutionPaths(req, res);
      }
      else if (apiPath.match(/^\/code-flow\/branches\/\d+$/) && req.method === 'GET') {
        const symbolId = apiPath.split('/')[3];
        req.params.symbolId = symbolId;
        await this.codeFlowRoutes.getBranchAnalysis(req, res);
      }
      else if (apiPath === '/code-flow/unused-paths' && req.method === 'GET') {
        await this.codeFlowRoutes.getUnusedPaths(req, res);
      }
      else if (apiPath.match(/^\/code-flow\/control-flow\/\d+$/) && req.method === 'GET') {
        const symbolId = apiPath.split('/')[3];
        req.params.symbolId = symbolId;
      ...
      else if (apiPath.match(/^\/code-flow\/analyze\/\d+$/) && req.method === 'GET') {
        const symbolId = apiPath.split('/')[3];
        req.params.symbolId = symbolId;
        await this.codeFlowRoutes.analyzeFunction(req, res);
      }
      else if (apiPath.match(/^\/code-flow\/complexity\/\d+$/) && req.method === 'GET') {
        const symbolId = apiPath.split('/')[3];
        req.params.symbolId = symbolId;
   ...
   */
  private createRequest(req: http.IncomingMessage, parsedUrl: url.UrlWithParsedQuery): Request {
    return {
      params: {},
   ...
   * Send error response
   */
  private sendErrorResponse(res: http.ServerResponse | Response, statusCode: number, message: string) {
  ...
  private async handleIndexProject(req: Request, res: Response) {
    try {
      ...
      if (req.headers['content-type']?.includes('application/json')) {
        // In a real implementation, you'd parse the request body here
        // For now, use default options
      }

      // Detect project languages automatically
      console.log(`🔍 Detecting languages for project: ${project.root_path}`);
      let detectedLanguages: string[];
      
      try {
        // Quick detection for responsiveness
        detectedLanguages = await LanguageDetectionService.quickDetectLanguages(project.root_path);
        
        if (detectedLanguages.length === 0) {
          detectedLanguages = ['cpp']; // Fallback to C++
        }
        
        console.log(`📋 Detected languages: ${detectedLanguages.join(', ')}`);
      } catch (error) {
        console.warn('Language detection failed, falling back to C++:', error);
        detectedLanguages = ['cpp'];
      }

      // Start indexing with detected languages
  ...
  private async handleGetIndexingJob(req: Request, res: Response) {
    try {
      ...
      const response: ApiResponse = {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to get indexing job'
      };
  ...
  private async handleGetIndexingJobs(req: Request, res: Response) {
    try {
      ...
      const response: ApiResponse = {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to get indexing jobs'
      };
  ...
  private async handleDetectProjectLanguages(req: Request, res: Response) {
    try {
      ...
      console.log(`🔍 Performing detailed language detection for: ${project.root_path}`);
      
      const languageProfile = await LanguageDetectionService.detectProjectLanguages(
        project.root_path,
      ...
      const response: ApiResponse = {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to detect project languages'
      };
   ...
   */
  private async handleCreateProject(req: Request, res: Response, rawReq?: http.IncomingMessage) {
    try {
   ...
   */
  private async handleUpdateProject(req: Request, res: Response, rawReq?: http.IncomingMessage) {
    try {
      const projectId = parseInt(req.params.id, 10);
  ...
  private async handleDeleteProject(req: Request, res: Response) {
    try {
      ...
      const response: ApiResponse = {
        ...
        message: hardDelete ? 'Project permanently deleted' : 'Project deactivated'
      };
