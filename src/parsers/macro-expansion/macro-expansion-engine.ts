...
import { UniversalSymbol, UniversalRelationship, UniversalRelationshipType } from '../language-parser-interface.js';
...
export class MacroExpansionEngine {
   ...
   */
  async discoverMacroDefinitions(projectPath: string, includePatterns: string[] = ['**/*.h', '**/*.hpp']): Promise<MacroDefinition[]> {
    ...
    if (this.debugMode) {
      console.log(`[MacroEngine] Discovered ${discovered.length} macros from ${this.headerFiles.size} header files`);
    }
   ...
   */
  private async extractMacrosFromFile(filePath: string): Promise<MacroDefinition[]> {
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        const defineMatch = line.match(/^#define\s+([A-Z_][A-Z0-9_]*)\s*(?:\(([^)]*)\))?\s+(.*)/);
        if (defineMatch) {
          ...
          let currentLine = i;
          while (fullExpansion.endsWith('\\') && currentLine + 1 < lines.length) {
            currentLine++;
            fullExpansion = fullExpansion.slice(0, -1) + ' ' + lines[currentLine].trim();
          }
          ...
          const macro: MacroDefinition = {
            ...
            sourceFile: filePath,
            line: i + 1,
            relationshipTemplates: this.generateRelationshipTemplates(name, fullExpansion),
          ...
          if (this.debugMode) {
            console.log(`[MacroEngine] Found macro ${name} in ${path.basename(filePath)}:${i + 1}`);
          }
        }
      }
      ...
      if (this.debugMode) {
        console.warn(`[MacroEngine] Failed to read ${filePath}: ${error}`);
      }
      return [];
    }
  }
   ...
   */
  private generateRelationshipTemplates(macroName: string, expansion: string): MacroRelationshipTemplate[] {
    ...
    if (macroName.startsWith('LOG_')) {
      ...
      const callChainMatch = expansion.match(/:*([A-Za-z_:]+)::getInstance\(\)\.(\w+)\(/);
      if (callChainMatch) {
    ...
    if (functionCalls) {
      for (const call of functionCalls) {
        ...
        if (funcName !== macroName && !funcName.match(/^(if|for|while|sizeof)$/)) {
          templates.push({
            fromSymbol: 'CALLER',
            toSymbol: funcName,
            type: UniversalRelationshipType.Calls,
  ...
  private calculateMacroComplexity(expansion: string): number {
    ...
    if (expansion.includes('do {') || expansion.includes('while(')) complexity += 1;
   ...
   */
  expandMacroInContext(macroName: string, parameters: string[], context: MacroExpansionContext): MacroExpansionResult | null {
    ...
    if (definition.isFunction && definition.parameters.length > 0) {
      for (let i = 0; i < definition.parameters.length && i < parameters.length; i++) {
  ...
  private generateRelationshipsFromTemplates(
    ...
    for (const template of templates) {
      ...
      const fromSymbolId = template.fromSymbol === 'CALLER' 
        ? context.callerSymbol?.qualifiedName || context.callerSymbol?.name || 'unknown'
        : template.fromSymbol;
  ...
  shouldSuppressSymbol(symbolName: string, _context: ParseContext): boolean {
    ...
    if (macro) {
      if (this.debugMode) {
        console.log(`[MacroEngine] Suppressing symbol ${symbolName} (it's a macro)`);
      }
      return true;
    }
    ...
    if (symbolName.startsWith('LOG_')) {
      if (this.debugMode) {
        console.log(`[MacroEngine] Suppressing LOG macro symbol ${symbolName}`);
      }
      return true;
    }
  ...
  private initializeBuiltinMacros(): void {
    ...
    const logMacros = [
      'LOG_ERROR', 'LOG_DEBUG', 'LOG_INFO', 'LOG_WARN', 
      'LOG_WARNING', 'LOG_CRITICAL', 'LOG_TRACE', 'LOG_FATAL'
    ];
    ...
    for (const macroName of logMacros) {
      if (!this.macroDefinitions.has(macroName)) {
        const macro: MacroDefinition = {
          ...
          expansion: `::Core::Logging::Logger::getInstance().${macroName.toLowerCase().replace('log_', '')}(LOG_CONTEXT(component), __VA_ARGS__)`,
          sourceFile: 'builtin',
          line: 0,
    ...
    if (this.debugMode) {
      console.log(`[MacroEngine] Initialized ${logMacros.length} built-in LOG macros`);
    }
  }
}