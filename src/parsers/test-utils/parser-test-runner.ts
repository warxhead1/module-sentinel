...
import { ParseOptions, SymbolInfo, RelationshipInfo } from '../tree-sitter/parser-types.js';
...
export class ParserTestRunner {
   ...
   */
  async testFile(filePath: string, options?: ParseOptions): Promise<TestResult> {
    const content = fs.readFileSync(filePath, 'utf-8');
    const language = this.detectLanguage(filePath);
    
    return this.testContent(content, language, path.basename(filePath), options);
  }
  ...
  async testContent(
    ...
    try {
      ...
      const parseResult = await parser.parseFile(`test.${this.getExtension(language)}`, content);
   ...
   */
  async runTestCases(testCases: TestCase[]): Promise<Record<string, TestResult>> {
   ...
   */
  private detectPitfalls(result: TestResult, content: string, language: string): void {
    ...
    if (language === 'typescript' || language === 'cpp') {
      ...
      if (nestedClasses && result.symbols.filter(s => s.kind === 'class').length < 2) {
        result.pitfallsDetected.push('NESTED_CLASS_MISSING: Parser may miss nested class definitions');
      }
    }
    
    // 2. Arrow functions not detected (TypeScript)
    if (language === 'typescript') {
      const arrowFunctions = content.match(/const\s+\w+\s*=\s*(?:async\s+)?(?:\([^)]*\)|[^=]+)\s*=>/g);
      const detectedArrowFuncs = result.symbols.filter(s => 
        s.kind === 'function' && s.languageFeatures?.isArrowFunction
      );
      
      if (arrowFunctions && detectedArrowFuncs.length < arrowFunctions.length) {
        result.pitfallsDetected.push('ARROW_FUNCTION_MISSING: Some arrow functions not detected');
      }
    }
    
    // 3. Template functions not detected (C++)
    if (language === 'cpp') {
      const templateFuncs = content.match(/template\s*<[^>]+>\s*(?:class|typename|auto|[^{]+)\s+\w+\s*\(/g);
      const detectedTemplates = result.symbols.filter(s => 
        s.signature?.includes('template') || s.name.includes('<')
      );
      
      if (templateFuncs && detectedTemplates.length === 0) {
        result.pitfallsDetected.push('TEMPLATE_MISSING: Template functions/classes not detected');
      }
    }
    ...
    if (content.match(spawnPatterns) || content.match(pythonCalls)) {
      ...
      if (crossLangRels.length === 0) {
        result.pitfallsDetected.push('CROSS_LANGUAGE_MISSING: Cross-language calls not detected');
      }
    }
    ...
    const fieldRelationships = result.relationships.filter(r => 
      r.relationshipType === 'reads_field' || r.relationshipType === 'writes_field'
    );
    
    if ((fieldAccess || fieldReads) && fieldRelationships.length === 0) {
      result.pitfallsDetected.push('FIELD_ACCESS_MISSING: Field read/write relationships not detected');
    }
    
    // 6. Missing function signatures
    const functionsWithoutSignatures = result.symbols.filter(s => 
      (s.kind === 'function' || s.kind === 'method') && !s.signature
    );
    
    if (functionsWithoutSignatures.length > 0) {
      result.pitfallsDetected.push(`SIGNATURE_MISSING: ${functionsWithoutSignatures.length} functions without signatures`);
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].match(/^\s*(def|function|void|int|auto)\s+\w+/)) {
        ...
        if (functionLines > 100) {
          result.warnings.push(`Large function detected (${functionLines} lines) - may impact parser performance`);
        }
      }
    }
    ...
    if (content.match(/[^\x00-\x7F]/)) {
      result.warnings.push('File contains non-ASCII characters - may affect parsing accuracy');
    }
  }
   ...
   */
  private validateSymbols(result: TestResult, expected: Array<{name: string; kind: string; qualifiedName?: string}>): void {
   ...
   */
  private validateRelationships(result: TestResult, expected: Array<{type: string; from: string; to: string}>): void {
    for (const exp of expected) {
      ...
      if (!found) {
        result.errors.push(`Missing expected relationship: ${exp.from} -[${exp.type}]-> ${exp.to}`);
        result.passed = false;
      }
    }
  }
   ...
   */
  private async createParser(language: string, options?: ParseOptions): Promise<OptimizedTreeSitterBaseParser> {
    ...
    switch (language) {
      ...
      case 'cpp':
        ...
        const cppParser = new CppLanguageParser({ language: 'cpp', version: '1.0.0' });
  ...
  private detectLanguage(filePath: string): 'typescript' | 'python' | 'cpp' {
    ...
    if (['.cpp', '.cc', '.cxx', '.hpp', '.h', '.hxx', '.ixx', '.c'].includes(ext)) return 'cpp';
  ...
  printResults(results: Record<string, TestResult>): void {
    ...
    for (const [name, result] of Object.entries(results)) {
      ...
      console.log(`   Parsing Mode: ${result.parsingMode}`);
      ...
      if (result.pitfallsDetected.length > 0) {
        console.log(`   ⚠️  Pitfalls Detected:`);
