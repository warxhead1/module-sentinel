...
export class APIUsageTracker {
  ...
  private async trackVulkanAPI(): Promise<void> {
    ...
    const vulkanCalls = this.db.prepare(`
    ...
    const vulkanRelationships = this.db.prepare(`
    ...
    console.log(`    Found ${vulkanCalls.length} Vulkan API calls + ${vulkanRelationships.length} relationship usages`);

    // Categorize Vulkan API usage
    const vulkanCategories = {
      buffer_management: ['VkBuffer', 'vkCreateBuffer', 'vkBindBufferMemory', 'vkMapMemory', 'vkUnmapMemory'],
      command_recording: ['VkCommandBuffer', 'vkBeginCommandBuffer', 'vkEndCommandBuffer', 'vkCmdDraw'],
      pipeline_creation: ['VkPipeline', 'vkCreateGraphicsPipelines', 'vkCreateComputePipelines'],
      descriptor_sets: ['VkDescriptorSet', 'vkCreateDescriptorSetLayout', 'vkUpdateDescriptorSets'],
      synchronization: ['VkSemaphore', 'VkFence', 'vkWaitForFences', 'vkCreateSemaphore'],
      memory_management: ['VkDeviceMemory', 'vkAllocateMemory', 'vkFreeMemory'],
      render_passes: ['VkRenderPass', 'vkCreateRenderPass', 'vkCmdBeginRenderPass'],
      swapchain: ['VkSwapchainKHR', 'vkCreateSwapchainKHR', 'vkAcquireNextImageKHR']
    };
    ...
    for (const [category, patterns] of Object.entries(vulkanCategories)) {
      ...
      console.log(`      ${category}: ${categoryUsages.length} usages`);
    }
  ...
  private async trackVulkanBufferLifecycle(): Promise<void> {
    ...
    const bufferCreators = this.db.prepare(`
    ...
    const bufferUsers = this.db.prepare(`
      SELECT s.id, s.qualified_name, s.parent_class
      FROM enhanced_symbols s
    ...
    for (const creator of bufferCreators) {
      for (const user of bufferUsers) {
        if (creator.parent_class === user.parent_class || 
    ...
    console.log(`      Buffer creators: ${bufferCreators.length}, Users: ${bufferUsers.length}`);
  }
  ...
  private async trackOpenGLAPI(): Promise<void> {
    ...
    const openglCalls = this.db.prepare(`
    ...
    console.log(`    Found ${openglCalls.length} OpenGL API calls`);
  }
  ...
  private async trackSTLUsage(): Promise<void> {
    ...
    for (const [category, patterns] of Object.entries(stlPatterns)) {
      ...
      console.log(`    ${category}: ${usages.length} usages`);
    }
  }
  ...
  private async trackFileSystemAPI(): Promise<void> {
    ...
    const fileAPIs = this.db.prepare(`
      SELECT id, qualified_name, signature
      FROM enhanced_symbols
    ...
    console.log(`    Found ${fileAPIs.length} file system API usages`);
  }
  ...
  private async trackThreadingAPI(): Promise<void> {
    ...
    const threadingAPIs = this.db.prepare(`
      SELECT id, qualified_name, signature
      FROM enhanced_symbols
    ...
    console.log(`    Found ${threadingAPIs.length} threading API usages`);
  }
...
if (require.main === module) {
  ...
  console.log(`🔍 Tracking API usage patterns in ${dbPath}...`);
  
  const tracker = new APIUsageTracker(dbPath);
  tracker.trackAllAPIUsage()
    .then(async () => {
      const stats = await tracker.getAPIUsageStats();
      console.log('\n📊 API Usage Statistics:');
      stats.forEach((stat: any) => {
        console.log(`  ${stat.semantic_tags}: ${stat.usage_count} usages`);
      });
