...
export class EnhancedIntelliSenseBridge {
  ...
  async generateArchitecturalGuidance(
    ...
    const navigation = await this.generateNavigationAids(
      context,
      currentContext
    );
  ...
  private async analyzeCurrentContext(
    context: IntelliSenseContext
  ): Promise<ArchitecturalGuidance["currentContext"]> {
    ...
    const filePatterns = patterns.filter(
      (p) => p.location.filePath === context.currentFile
    );

    // Calculate complexity and maintainability
    const complexity = this.calculateFileComplexity(fileInfo, filePatterns);
    const maintainability = this.calculateFileMaintainability(
      fileInfo,
      filePatterns
    );
  ...
  private async generateSmartSuggestions(
    ...
    suggestions.push(
      ...(await this.generatePatternSuggestions(context, currentContext))
    );

    // Architectural suggestions
    suggestions.push(
      ...(await this.generateArchitecturalSuggestions(context, currentContext))
    );

    // Optimization suggestions
    suggestions.push(
      ...(await this.generateOptimizationSuggestions(context, currentContext))
    );

    // Best practice suggestions
    suggestions.push(
      ...(await this.generateBestPracticeSuggestions(context, currentContext))
    );

    // Warning suggestions
    suggestions.push(
      ...(await this.generateWarningSuggestions(context, currentContext))
    );
  ...
  private async generatePatternSuggestions(
      ...
      for (const pattern of currentContext.patterns) {
        if (pattern.confidence < 0.9) {
          suggestions.push({
            ...
            description: `The ${pattern.name} pattern is incomplete (${(
              pattern.confidence * 100
            ).toFixed(
              1
            )}% confidence). Consider implementing missing components.`,
            action: {
              type: "refactor",
              target: pattern.name,
            ...
            documentation: `${
              pattern.patternType
            } patterns typically require: ${pattern.relationships.join(", ")}`,
          });
        }
      }
    }
    ...
    if (
      currentContext.stage === "rendering" &&
      currentContext.patterns.length === 0
    ) {
      suggestions.push({
        ...
        description:
          "Rendering stage files benefit from RAII patterns for automatic resource management.",
        action: {
          type: "code-completion",
          suggestion: "Implement RAII wrapper for Vulkan resources",
        },
        reasoning:
          "Vulkan resources require careful lifecycle management, RAII patterns prevent resource leaks",
        confidence: 0.85,
        context: {
          file: context.currentFile,
...
public:
  ...
  private async generateArchitecturalSuggestions(
    ...
    if (crossStageDeps.length > 0) {
      ...
      suggestions.push({
        ...
        reasoning:
          "Cross-stage dependencies can indicate architectural coupling issues",
        confidence: 0.75,
        context: {
          file: context.currentFile,
          patterns: [],
    ...
    if (currentContext.complexity > 7) {
      suggestions.push({
        ...
        action: {
          type: "refactor",
          suggestion:
            "Break down complex components into smaller, focused modules",
        },
        reasoning:
  ...
  private async generateOptimizationSuggestions(
    ...
    if (currentContext.stage === "rendering") {
      const hasGPUPatterns = currentContext.patterns.some((p) =>
        p.semanticTags.includes("gpu")
      );
      if (!hasGPUPatterns) {
        suggestions.push({
          id: "gpu_optimization",
          ...
          description:
            "Rendering stage files can benefit from GPU compute patterns for better performance.",
          action: {
            type: "code-completion",
            suggestion: "Implement GPU compute shaders for parallel processing",
          },
          reasoning:
            "GPU parallelization can significantly improve rendering performance",
          confidence: 0.7,
          context: {
            file: context.currentFile,
    ...
    const hasMemoryPatterns = currentContext.patterns.some(
      (p) => p.patternType === "memory-pool"
    );
    if (!hasMemoryPatterns && currentContext.stage !== "gui") {
      suggestions.push({
        ...
        description:
          "Memory pool patterns can improve performance for frequent allocations.",
        action: {
          type: "code-completion",
  ...
  private async generateBestPracticeSuggestions(
    ...
    const modernCppSymbols = this.db
      .prepare(
        `
    ...
    if (modernCppSymbols.count === 0) {
      suggestions.push({
        ...
        description:
          "This file could benefit from modern C++20/23 features like concepts, modules, or constexpr.",
        action: {
          type: "refactor",
          suggestion:
            "Adopt modern C++ features for better type safety and performance",
        },
        reasoning:
          "Modern C++ features improve code safety, readability, and performance",
        confidence: 0.6,
        context: {
          file: context.currentFile,
          patterns: [],
  ...
  private async generateWarningSuggestions(
    ...
    if (antiPatterns.length > 0) {
      suggestions.push({
        ...
        description: `Found ${antiPatterns.length} anti-patterns: ${antiPatterns
          .slice(0, 2)
          .join(", ")}`,
        action: {
          type: "refactor",
        ...
        reasoning:
          "Anti-patterns indicate design issues that should be addressed",
        confidence: 0.85,
        context: {
          file: context.currentFile,
          patterns: [],
    ...
    // Confidence warnings
    const lowConfidenceSymbols = this.db
      .prepare(
        `
      SELECT COUNT(*) as count
      FROM universal_symbols
    ...
    if (lowConfidenceSymbols.count > 0) {
      suggestions.push({
        ...
        reasoning:
          "Low parser confidence may indicate ambiguous or complex code",
        confidence: 0.7,
        context: {
          file: context.currentFile,
          patterns: [],
  ...
  private async generateInsights(
    ...
    if (currentContext.maintainability > 80) {
      strengths.push(
        "High maintainability score indicates well-structured code"
      );
    }

    if (currentContext.patterns.length > 0) {
      strengths.push(
        `Implements ${currentContext.patterns.length} architectural patterns`
      );
    }
    ...
    const antiPatternCount = currentContext.patterns.reduce(
    ...
    if (
      currentContext.stage === "rendering" &&
      !currentContext.patterns.some((p) => p.patternType === "vulkan-raii")
    ) {
    ...
    if (currentContext.patterns.length === 0) {
      opportunities.push(
        "Consider implementing architectural patterns for better structure"
      );
    }

    if (currentContext.complexity > 5 && currentContext.patterns.length === 0) {
      opportunities.push(
        "Refactoring into design patterns could reduce complexity"
      );
    }
  ...
  private calculateFileComplexity(
    ...
    complexity +=
      patterns.reduce((sum, p) => sum + p.complexity, 0) /
  ...
  private calculateFileMaintainability(
    ...
    maintainability -= patterns.reduce(
      (sum, p) => sum + p.antiPatterns.length * 10,
      0
    );
  ...
  async enhanceMCPTool(
    ...
    const suggestions = await this.generateToolSpecificSuggestions(
      toolName,
      guidance,
      context
    );
    ...
    switch (toolName) {
      case "find_implementations":
        suggestions.push({
          ...
          description:
            "Search includes related pattern implementations and architectural variants",
          action: {
            type: "navigate",
            parameters: {
              includePatterns: guidance.currentContext.patterns.map(
                (p) => p.patternType
              ),
          ...
          reasoning:
            "Architectural context helps find more relevant implementations",
          confidence: 0.8,
          context: {
            file: context.currentFile,
            patterns: guidance.currentContext.patterns.map(
              (p) => p.patternType
            ),
      ...
      case "analyze_impact":
        if (context.currentSymbol) {
          suggestions.push({
            ...
            description:
              "Enhanced impact analysis with change scenario modeling",
            action: {
              type: "analyze",
            ...
            reasoning:
              "Predictive analysis helps understand long-term impact of changes",
            confidence: 0.9,
            context: {
              file: context.currentFile,
      ...
      case "get_api_surface":
        suggestions.push({
          ...
          description:
            "API surface analysis includes pattern interfaces and architectural boundaries",
          action: {
            type: "analyze",
            parameters: {
          ...
          reasoning:
            "Pattern-aware analysis provides better API organization insights",
          confidence: 0.85,
          context: {
            file: context.currentFile,
            patterns: guidance.currentContext.patterns.map(
  ...
  async getRealtimeSuggestions(
    ...
    if (
      partialCode.includes("class") &&
      context.semanticContext.stage === "rendering"
    ) {
      suggestions.push({
