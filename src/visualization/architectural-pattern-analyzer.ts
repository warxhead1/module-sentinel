...
export interface PatternInstance {
  id: string;
  patternType: 'factory' | 'singleton' | 'observer' | 'strategy' | 'adapter' | 'facade' | 'builder' | 'command' | 'pipeline' | 'vulkan-raii' | 'gpu-compute' | 'memory-pool';
  name: string;
  confidence: number;
  location: {
    filePath: string;
...
export interface PatternRelationship {
  source: string;
  target: string;
  relationType: 'uses' | 'extends' | 'composes' | 'delegates' | 'coordinates' | 'conflicts';
  strength: number;        // 0-1 strength of relationship
  description: string;
}

export interface ArchitecturalInsight {
  type: 'pattern_cluster' | 'anti_pattern_hotspot' | 'design_drift' | 'complexity_spike' | 'coupling_issue';
...
export class ArchitecturalPatternAnalyzer {
  ...
  async analyzePatterns(): Promise<PatternInstance[]> {
    ...
    console.log(`📊 Found ${patterns.length} architectural patterns`);
    return patterns;
  }
  ...
  private async analyzeFactoryPatterns(): Promise<PatternInstance[]> {
    ...
    for (const symbol of factorySymbols) {
      ...
      const pattern: PatternInstance = {
        ...
        participants: [symbol.qualified_name, ...products.map((p: any) => p.qualified_name)],
        relationships: products.map((p: any) => `creates ${p.name}`),
        semanticTags: symbol.semantic_tags ? symbol.semantic_tags.split(',') : [],
        complexity,
        maintainabilityScore: this.calculateMaintainabilityScore(symbol, products.length),
  ...
  private async analyzeSingletonPatterns(): Promise<PatternInstance[]> {
    const singletonSymbols = this.db.prepare(`
      SELECT 
        id, name, qualified_name, file_path, location_line, location_column,
        pipeline_stage, semantic_tags, parser_confidence
      FROM enhanced_symbols
  ...
  private async analyzeObserverPatterns(): Promise<PatternInstance[]> {
    const observerSymbols = this.db.prepare(`
      SELECT 
        id, name, qualified_name, file_path, location_line, location_column,
        pipeline_stage, semantic_tags, parser_confidence
      FROM enhanced_symbols
    ...
    for (const symbol of observerSymbols) {
      ...
      const notifyMethods = this.db.prepare(`
        SELECT COUNT(*) as count
        FROM enhanced_symbols
      ...
      if (notifyMethods.count > 0) {
        const observers = this.db.prepare(`
        ...
        const pattern: PatternInstance = {
          ...
          stage: symbol.pipeline_stage || 'unknown',
          participants: [symbol.qualified_name, ...observers.map((o: any) => o.qualified_name)],
          relationships: ['notifies observers', 'maintains observer list'],
          semanticTags: symbol.semantic_tags ? symbol.semantic_tags.split(',') : [],
          complexity: Math.min(10, 3 + observers.length * 0.3),
          maintainabilityScore: this.calculateMaintainabilityScore(symbol, observers.length),
  ...
  private async analyzeStrategyPatterns(): Promise<PatternInstance[]> {
    const strategySymbols = this.db.prepare(`
      SELECT 
        id, name, qualified_name, file_path, location_line, location_column,
        pipeline_stage, semantic_tags, parser_confidence
      FROM enhanced_symbols
    ...
    for (const symbol of strategySymbols) {
      ...
      const pattern: PatternInstance = {
        ...
        stage: symbol.pipeline_stage || 'unknown',
        participants: [symbol.qualified_name, ...implementations.map((i: any) => i.qualified_name)],
        relationships: ['defines algorithm interface', 'enables runtime algorithm selection'],
        semanticTags: symbol.semantic_tags ? symbol.semantic_tags.split(',') : [],
        complexity: Math.min(10, 2 + implementations.length * 0.4),
        maintainabilityScore: this.calculateMaintainabilityScore(symbol, implementations.length),
  ...
  private async analyzeVulkanRAIIPatterns(): Promise<PatternInstance[]> {
    const vulkanSymbols = this.db.prepare(`
      SELECT 
        id, name, qualified_name, file_path, location_line, location_column,
        pipeline_stage, semantic_tags, parser_confidence
      FROM enhanced_symbols
    ...
    for (const symbol of vulkanSymbols) {
      ...
      const hasVulkanCalls = this.db.prepare(`
        SELECT COUNT(*) as count
        FROM enhanced_symbols
        WHERE parent_class = ? 
      ...
      if (hasVulkanCalls.count > 0) {
        const pattern: PatternInstance = {
          ...
          participants: [symbol.qualified_name],
          relationships: ['manages vulkan resource lifecycle', 'provides exception safety'],
          semanticTags: symbol.semantic_tags ? symbol.semantic_tags.split(',') : [],
  ...
  private async analyzeGPUComputePatterns(): Promise<PatternInstance[]> {
    ...
    for (const symbol of gpuSymbols) {
      const pattern: PatternInstance = {
        ...
        stage: symbol.pipeline_stage || 'rendering',
        participants: [symbol.qualified_name],
        relationships: ['orchestrates gpu computation', 'manages compute shaders'],
        semanticTags: symbol.semantic_tags ? symbol.semantic_tags.split(',') : [],
  ...
  private async analyzeMemoryPoolPatterns(): Promise<PatternInstance[]> {
    const memorySymbols = this.db.prepare(`
      SELECT 
        id, name, qualified_name, file_path, location_line, location_column,
        pipeline_stage, semantic_tags, parser_confidence
      FROM enhanced_symbols
    ...
    for (const symbol of memorySymbols) {
      const pattern: PatternInstance = {
        ...
        stage: symbol.pipeline_stage || 'unknown',
        participants: [symbol.qualified_name],
        relationships: ['manages memory allocation', 'provides performance optimization'],
        semanticTags: symbol.semantic_tags ? symbol.semantic_tags.split(',') : [],
  ...
  private async analyzePipelinePatterns(): Promise<PatternInstance[]> {
    const pipelineSymbols = this.db.prepare(`
      SELECT 
        id, name, qualified_name, file_path, location_line, location_column,
        pipeline_stage, semantic_tags, parser_confidence
      FROM enhanced_symbols
    ...
    for (const symbol of pipelineSymbols) {
      ...
      const pattern: PatternInstance = {
        ...
        participants: [symbol.qualified_name, ...stages.map((s: any) => s.qualified_name)],
        relationships: ['coordinates processing stages', 'manages data flow'],
        semanticTags: symbol.semantic_tags ? symbol.semantic_tags.split(',') : [],
        complexity: Math.min(10, 4 + stages.length * 0.5),
        maintainabilityScore: this.calculateMaintainabilityScore(symbol, stages.length),
  ...
  private calculateMaintainabilityScore(symbol: any, participantCount: number): number {
  ...
  private detectFactoryAntiPatterns(symbol: any, products: any[]): string[] {
    ...
    if (products.length > 1) {
      const hasCommonInterface = products.some((p: any) => 
        products.some((other: any) => 
          other !== p && other.qualified_name.includes(p.name.split('Base')[0])
        )
      );
  ...
  private detectStrategyAntiPatterns(symbol: any, implementations: any[]): string[] {
    const antiPatterns: string[] = [];
  ...
  private detectVulkanAntiPatterns(symbol: any): string[] {
    ...
    // Resource Leaks
    if (!symbol.semantic_tags?.includes('cleanup') && !symbol.semantic_tags?.includes('raii')) {
      antiPatterns.push('resource-leak-risk');
    }
    
    // Synchronization Issues
    if (symbol.semantic_tags?.includes('threading') && !symbol.semantic_tags?.includes('synchronized')) {
  ...
  private detectGPUAntiPatterns(symbol: any): string[] {
    ...
    // CPU-GPU Sync Issues
    if (!symbol.semantic_tags?.includes('async') && symbol.semantic_tags?.includes('blocking')) {
  ...
  private async calculatePatternMetrics(symbolId: number): Promise<PatternInstance['metrics']> {
    // Get relationship counts for fan-in/fan-out
    const fanIn = this.db.prepare(`
      SELECT COUNT(*) as count
      FROM symbol_relationships
      WHERE to_symbol_id = ?
    `).get(symbolId) as any;

    const fanOut = this.db.prepare(`
      SELECT COUNT(*) as count
      FROM symbol_relationships
  ...
  private async getPatternEvolution(symbolId: number): Promise<PatternInstance['evolution']> {
    // Simplified evolution tracking - in reality this would track git history
    return {
      firstSeen: new Date(Date.now() - Math.random() * 365 * 24 * 60 * 60 * 1000),
      lastModified: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000),
      changeCount: Math.floor(Math.random() * 20),
      stabilityTrend: ['improving', 'degrading', 'stable'][Math.floor(Math.random() * 3)] as any
    };
  }
   ...
   */
  async generateInsights(patterns: PatternInstance[]): Promise<ArchitecturalInsight[]> {
  ...
  private analyzePatternClusters(patterns: PatternInstance[]): ArchitecturalInsight[] {
    const insights: ArchitecturalInsight[] = [];
  ...
  private analyzeAntiPatternHotspots(patterns: PatternInstance[]): ArchitecturalInsight[] {
    const insights: ArchitecturalInsight[] = [];
    
    // Find patterns with multiple anti-patterns
    const problematicPatterns = patterns.filter(p => p.antiPatterns.length > 1);
    
    if (problematicPatterns.length > 0) {
      insights.push({
        type: 'anti_pattern_hotspot',
        severity: Math.min(10, problematicPatterns.length + 
  ...
  private analyzeDesignDrift(patterns: PatternInstance[]): ArchitecturalInsight[] {
    const insights: ArchitecturalInsight[] = [];
    
    // Find patterns with degrading stability
    const degradingPatterns = patterns.filter(p => p.evolution.stabilityTrend === 'degrading');
  ...
  private analyzeComplexitySpikes(patterns: PatternInstance[]): ArchitecturalInsight[] {
    const insights: ArchitecturalInsight[] = [];
    ...
    if (complexPatterns.length > 0) {
      insights.push({
        ...
        severity: Math.min(8, complexPatterns.length + 
          Math.max(...complexPatterns.map(p => p.complexity)) - 5),
  ...
  private analyzeCouplingIssues(patterns: PatternInstance[]): ArchitecturalInsight[] {
    const insights: ArchitecturalInsight[] = [];
    
    // Find patterns with high coupling
    const tightlyCoupledPatterns = patterns.filter(p => p.metrics.coupling > 7);
   ...
   */
  async generatePatternRelationships(patterns: PatternInstance[]): Promise<PatternRelationship[]> {
      ...
      for (let j = i + 1; j < patterns.length; j++) {
        ...
        if (sharedParticipants.length > 0) {
          relationships.push({
            ...
            strength: sharedParticipants.length / Math.max(pattern1.participants.length, pattern2.participants.length),
            description: `Shares ${sharedParticipants.length} participants: ${sharedParticipants.slice(0, 2).join(', ')}`
          });
        }
        
        // Check for stage-based relationships
        if (pattern1.stage === pattern2.stage && sharedParticipants.length === 0) {
          relationships.push({
            source: pattern1.id,
            target: pattern2.id,
  ...
  private determineRelationType(pattern1: PatternInstance, pattern2: PatternInstance): PatternRelationship['relationType'] {
    // Factory + Strategy often work together
    if ((pattern1.patternType === 'factory' && pattern2.patternType === 'strategy') ||
        (pattern1.patternType === 'strategy' && pattern2.patternType === 'factory')) {
      return 'composes';
    }
    
    // Observer + Singleton often compose
    if ((pattern1.patternType === 'observer' && pattern2.patternType === 'singleton') ||
        (pattern1.patternType === 'singleton' && pattern2.patternType === 'observer')) {
      return 'uses';
    }
    
    // Vulkan patterns often extend each other
    if (pattern1.patternType.includes('vulkan') && pattern2.patternType.includes('vulkan')) {
      return 'extends';
    }
...
<body>
    <div class="container">
        <div class="sidebar">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-value">${insights.filter(i => i.severity >= 7).length}</div>
                    <div class="stat-label">High Severity Issues</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value">${Math.round(patterns.reduce((sum, p) => sum + p.maintainabilityScore, 0) / patterns.length)}</div>
                    <div class="stat-label">Avg Maintainability</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value">${Object.keys(stageStats).length}</div>
            ...
            <div class="insights-section">
                ...
                ${insights.slice(0, 5).map(insight => `
                    <div class="insight ${insight.severity >= 7 ? 'high-severity' : insight.severity >= 4 ? 'medium-severity' : 'low-severity'}">
            ...
            <div class="pattern-types">
                <h3>Pattern Types</h3>
                ${Object.entries(patternTypeStats).map(([type, count]) => `
                    <div class="pattern-type" onclick="filterByType('${type}')">
                        <span class="pattern-type-name">${type.replace('-', ' ').toUpperCase()}</span>
  ...
  private calculateStageStatistics(patterns: PatternInstance[]): Record<string, number> {
  ...
  private calculatePatternTypeStatistics(patterns: PatternInstance[]): Record<string, number> {
    const stats: Record<string, number> = {};
    for (const pattern of patterns) {
