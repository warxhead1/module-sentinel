...
export class RippleEffectTracker {
  ...
  private initializeGraphs(): void {
    ...
    try {
      ...
      for (const rel of relationships) {
        ...
        this.reverseDependencyGraph.get(rel.target_symbol)!.add(rel.source_symbol);
      }
    } catch (error) {
  ...
  async predictImpact(
    ...
    console.log(`🌊 Analyzing ripple effects for ${symbolName} (${changeType})`);
    ...
    const recommendations = this.generateRecommendations(symbol, changeType, affectedNodes);
  ...
  private async getSymbolDetails(symbolName: string): Promise<RippleNode | null> {
    try {
      ...
      // Get dependencies
      const dependencies = this.dependencyGraph.get(symbol.qualified_name) || new Set();
      const dependents = this.reverseDependencyGraph.get(symbol.qualified_name) || new Set();
      ...
      return {
        ...
        criticalityScore: this.calculateCriticalityScore(symbol, usageCount?.count || 0),
  ...
  private async findAffectedNodes(
    ...
    while (queue.length > 0) {
      ...
      if (depth > 0) { // Don't include the source symbol itself
        const impactSeverity = this.calculateImpactSeverity(node, changeType, depth);
        const requiredActions = this.determineRequiredActions(node, changeType);
        const estimatedFixTime = this.estimateFixTime(node, changeType, impactSeverity);
  ...
  private calculateImpactLevel(symbol: any): number {
    ...
    if (symbol.semantic_tags?.includes('core') || 
  ...
  private calculateCriticalityScore(symbol: any, usageCount: number): number {
    ...
    if (symbol.file_path?.includes('include/') || symbol.file_path?.endsWith('.ixx')) {
      score += 1;
    }
  ...
  private calculateImpactSeverity(node: RippleNode, changeType: string, depth: number): number {
  ...
  private determineRequiredActions(node: RippleNode, changeType: string): string[] {
  ...
  private estimateFixTime(node: RippleNode, changeType: string, severity: number): number {
  ...
  private calculateRiskAssessment(affectedNodes: ImpactPrediction['affectedNodes']): ImpactPrediction['riskAssessment'] {
    const high = affectedNodes.filter(n => n.impactSeverity >= 7).length;
    const medium = affectedNodes.filter(n => n.impactSeverity >= 4 && n.impactSeverity < 7).length;
    ...
    const testingRequired = [
      ...new Set(affectedNodes.flatMap(n => n.requiredActions.filter(a => a.includes('test'))))
    ];
  ...
  private determineReviewersNeeded(affectedNodes: ImpactPrediction['affectedNodes']): string[] {
  ...
  private generateRecommendations(
    ...
    const totalFixTime = affectedNodes.reduce((sum, n) => sum + n.estimatedFixTime, 0);
    if (totalFixTime > 120) { // 2 hours
