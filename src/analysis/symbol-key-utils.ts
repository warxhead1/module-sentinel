 ...
 */
export function generateSemanticContextKey(symbol: SymbolInfo, filePath?: string): string {
 ...
 */
export function generateEmbeddingSymbolId(symbol: SymbolInfo, filePath?: string): string {
  return generateSymbolKey(symbol, filePath);
}
 ...
 */
export function generateDatabaseLookupKey(symbol: SymbolInfo, filePath?: string): string {
  return generateSymbolKey(symbol, filePath);
}
...
export function generateAlternativeKey(symbol: SymbolInfo, filePath?: string): string {
  ...
  return `${file}:${symbol.name || symbol.qualifiedName}:${symbol.line}:${symbol.column}`;
}

/**
 * Extract symbol info from a generated key (for debugging/reverse lookup)
 */
export function parseSymbolKey(key: string): { name: string; filePath: string; line: number; column: number } | null {
  try {
 ...
 */
export function debugKeyGeneration(symbol: SymbolInfo, filePath?: string, componentName?: string): void {
  const key = generateSymbolKey(symbol, filePath);
  const component = componentName ? `[${componentName}] ` : '';
  console.log(`${component}Generated key for ${symbol.name}: ${key}`);
}