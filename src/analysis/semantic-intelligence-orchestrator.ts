...
import {
  ...
  SemanticContext,
} from "./semantic-context-engine.js";
import {
...
import {
  ...
  SemanticInsight,
} from "./semantic-insights-generator.js";
import {
  SymbolInfo,
  RelationshipInfo,
...
export class SemanticIntelligenceOrchestrator {
  ...
  constructor(
    ...
    try {
      ...
      db.prepare(
        "CREATE TEMP TABLE IF NOT EXISTS test_write (id INTEGER)"
      ).run();
      db.prepare("DROP TABLE IF EXISTS test_write").run();
    } catch (error) {
      console.warn(
        "[SemanticOrchestrator] Database is read-only, semantic intelligence will run in limited mode"
      );
      isReadOnly = true;
    }
    ...
    try {
      this.contextEngine = new SemanticContextEngine(db, {
      ...
      console.log(
        "[SemanticOrchestrator] All semantic engines initialized successfully"
      );
    } catch (error) {
      console.error(
        "[SemanticOrchestrator] Failed to initialize semantic engines:",
        error
      );
      throw error;
    }
      ...
      new Promise<SemanticIntelligenceResult>((_, reject) =>
        setTimeout(
          () =>
            reject(
              new Error(
                `Semantic processing timeout after ${semanticProcessingTimeout}ms`
              )
            ),
          semanticProcessingTimeout
        )
      ),
    ]);
  }
      ...
      try {
        for (const symbol of symbols) {
          try {
            ...
            const { generateSemanticContextKey } = await import(
              "./symbol-key-utils.js"
            );
            ...
            console.warn(
              `[SemanticOrchestrator] Context extraction failed for ${symbol.name}:`,
              error
            );
        ...
        console.error(
          `[SemanticOrchestrator] Context extraction loop failed:`,
          error
        );
    ...
    if (config.enableEmbeddingGeneration && symbols.length > 0) {
      ...
      const batchData = symbols.map((symbol) => ({
        ...
        relationships: relationships.filter(
          (r) =>
            r.fromName === symbol.qualifiedName ||
            r.toName === symbol.qualifiedName
        ),
      }));

      try {
        console.log(
          `[SemanticOrchestrator] Attempting to generate embeddings for ${batchData.length} symbols`
        );

        // Add timeout for embedding generation
        const embeddingPromise =
          this.embeddingEngine.generateBatchEmbeddings(batchData);
          ...
          new Promise<CodeEmbedding[]>((_, reject) =>
            setTimeout(
              () =>
                reject(
                  new Error(
                    `Embedding generation timeout after ${embeddingTimeout}ms`
                  )
                ),
              embeddingTimeout
            )
        ...
        if (this.debugMode) {
          console.error(
            "[SemanticOrchestrator] Error stack:",
        ...
        const clusterResults = await Promise.race([
          ...
          new Promise<SemanticCluster[]>((_, reject) =>
            setTimeout(
              () =>
                reject(
                  new Error(`Clustering timeout after ${clusteringTimeout}ms`)
                ),
              clusteringTimeout
            )
          ),
          ...
          new Promise<SemanticInsight[]>((_, reject) =>
            setTimeout(
              () =>
                reject(
                  new Error(
                    `Insight generation timeout after ${insightTimeout}ms`
                  )
                ),
              insightTimeout
            )
    ...
    for (let i = 0; i < fileData.length; i += batchSize) {
      ...
      for (const result of batchResults) {
        ...
        for (const embedding of result.embeddings) {
          const existingIndex = allEmbeddings.findIndex(
            (e) => e.symbolId === embedding.symbolId
          );
          if (existingIndex === -1) {
