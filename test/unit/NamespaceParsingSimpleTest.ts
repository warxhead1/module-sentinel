...
export class NamespaceParsingSimpleTest extends BaseTest {
  ...
  private async testNamespaceStackManagement(): Promise<TestResult> {
    ...
    try {
      ...
      this.assertEqual(namespaceStack.join('::'), 'PlanetGen', 'Single namespace');
      
      this.enterNamespace(namespaceStack, 'Rendering');
      this.assertEqual(namespaceStack.join('::'), 'PlanetGen::Rendering', 'Nested namespace');
      
      this.enterNamespace(namespaceStack, 'SPIRV');
      this.assertEqual(namespaceStack.join('::'), 'PlanetGen::Rendering::SPIRV', 'Triple nested namespace');
      
      // Simulate exiting namespaces
      this.exitNamespace(namespaceStack);
      this.assertEqual(namespaceStack.join('::'), 'PlanetGen::Rendering', 'Exit one namespace');
      
      this.exitNamespace(namespaceStack);
      this.assertEqual(namespaceStack.join('::'), 'PlanetGen', 'Exit second namespace');
      
      this.exitNamespace(namespaceStack);
      this.assertEqual(namespaceStack.join('::'), '', 'Exit to global namespace');
  ...
  private async testExportNamespaceRecognition(): Promise<TestResult> {
    ...
    try {
      ...
      for (let i = 0; i < testPatterns.length; i++) {
        ...
        this.assertEqual(result.isExport, expected.isExport, `Export detection for: ${pattern}`);
        this.assertEqual(result.namespace, expected.namespace, `Namespace extraction for: ${pattern}`);
      }
  ...
  private async testMultiLevelNamespaceParsing(): Promise<TestResult> {
    ...
    try {
      ...
      const testCases = [
        ...
        { input: 'std::chrono::high_resolution_clock', expected: ['std', 'chrono', 'high_resolution_clock'] },
      ];
      ...
      for (const testCase of testCases) {
        ...
        if (result.length !== testCase.expected.length) {
          throw new Error(`Length mismatch for ${testCase.input}: expected ${testCase.expected.length}, got ${result.length}`);
        }
        
        for (let i = 0; i < result.length; i++) {
          this.assertEqual(result[i], testCase.expected[i], `Component ${i} of ${testCase.input}`);
        }
      }
  ...
  private async testScopeBoundaryTracking(): Promise<TestResult> {
    ...
    try {
      ...
      for (let i = 0; i < codeLines.length; i++) {
        ...
        this.assertEqual(actualStackStr, expectedStackStr, `Line ${i + 1}: "${line}"`);
      }
  ...
  private async testQualifiedNameBuilding(): Promise<TestResult> {
    ...
    try {
      ...
      const testCases = [
        { namespaceStack: [], className: 'GlobalClass', expected: 'GlobalClass' },
        { namespaceStack: ['A'], className: 'ClassA', expected: 'A::ClassA' },
        { namespaceStack: ['A', 'B'], className: 'ClassB', expected: 'A::B::ClassB' },
        { namespaceStack: ['PlanetGen', 'Rendering'], className: 'VulkanManager', expected: 'PlanetGen::Rendering::VulkanManager' },
        { namespaceStack: ['PlanetGen', 'Rendering', 'SPIRV'], className: 'SPIRVCore', expected: 'PlanetGen::Rendering::SPIRV::SPIRVCore' },
      ];

      for (const testCase of testCases) {
        const result = this.buildQualifiedName(testCase.namespaceStack, testCase.className);
        this.assertEqual(result, testCase.expected, `Qualified name for ${testCase.className} in [${testCase.namespaceStack.join(', ')}]`);
      }
  ...
  private async testNamespaceExtraction(): Promise<TestResult> {
    ...
    try {
      ...
      const testCases = [
        ...
        { qualifiedName: 'PlanetGen::Rendering::VulkanManager', expected: 'PlanetGen::Rendering' },
        { qualifiedName: 'PlanetGen::VulkanManager::VulkanManager', expected: 'PlanetGen' }, // Constructor pattern
      ];

      for (const testCase of testCases) {
        const className = testCase.qualifiedName.split('::').pop() || '';
        const result = this.extractNamespaceFromQualified(testCase.qualifiedName, className);
        this.assertEqual(result, testCase.expected, `Namespace extraction from ${testCase.qualifiedName}`);
      }
  ...
  private parseNamespaceDeclaration(line: string): { isExport: boolean; namespace: string } | null {
  ...
  private processLineForScopeTracking(line: string, namespaceStack: string[]): void {
  ...
  private buildQualifiedName(namespaceStack: string[], className: string): string {
  ...
  private extractNamespaceFromQualified(qualifiedName: string, className: string): string {
    // Handle constructor pattern: PlanetGen::ClassName::ClassName
    if (qualifiedName.endsWith('::' + className)) {
      const withoutConstructor = qualifiedName.substring(0, qualifiedName.length - ('::' + className).length);
      if (withoutConstructor.endsWith('::' + className)) {
        return withoutConstructor.substring(0, withoutConstructor.length - ('::' + className).length);
      }
    ...
    if (qualifiedName.endsWith(className)) {
      const namespace = qualifiedName.substring(0, qualifiedName.length - className.length - 2); // -2 for ::
      return namespace || '';
    }
  ...
  private async testRealFileNamespaceParsing(): Promise<TestResult> {
    ...
    try {
      ...
      const testFilePath = path.join(process.cwd(), 'test/complex-files/src-buffer/BufferFactory.cpp');
      ...
      const hasNamespaceDeclaration = content.includes('namespace PlanetGen::Rendering');
      if (!hasNamespaceDeclaration) {
        throw new Error(`Test file ${testFilePath} does not contain expected namespace declaration`);
      }
      
      console.log(`[NAMESPACE_TEST] Testing file: ${testFilePath}`);
      console.log(`[NAMESPACE_TEST] File contains namespace declaration: ${hasNamespaceDeclaration}`);
      ...
      const result = await parserAny.performPatternBasedExtraction(content, testFilePath);
      const symbols = result.symbols;
      
      console.log(`[NAMESPACE_TEST] Parsed ${symbols.length} symbols`);
      
      // Find symbols that should have namespace information
      const symbolsWithNamespaces = symbols.filter((s: any) => s.namespace && s.namespace !== '');
      console.log(`[NAMESPACE_TEST] Found ${symbolsWithNamespaces.length} symbols with namespaces`);
      
      if (symbolsWithNamespaces.length > 0) {
        symbolsWithNamespaces.forEach((s: any) => {
          console.log(`[NAMESPACE_TEST] Symbol: ${s.name}, Namespace: ${s.namespace}, Qualified: ${s.qualifiedName}`);
        });
      }
      
      // We expect to find at least some symbols in the PlanetGen::Rendering namespace
      const renderingNamespaceSymbols = symbols.filter((s: any) => 
        s.namespace === 'PlanetGen::Rendering' || 
        s.qualifiedName.includes('PlanetGen::Rendering')
      );
      
      console.log(`[NAMESPACE_TEST] Found ${renderingNamespaceSymbols.length} symbols in PlanetGen::Rendering namespace`);
      
      // Always list all symbols for debugging
      console.log(`[NAMESPACE_TEST] All symbols found:`);
      symbols.forEach((s: any, i: number) => {
        console.log(`[NAMESPACE_TEST] ${i + 1}. ${s.name} (${s.kind}) - namespace: '${s.namespace || 'null'}' - qualified: '${s.qualifiedName}'`);
      });
      
      if (renderingNamespaceSymbols.length === 0) {
        throw new Error(`Expected to find symbols in PlanetGen::Rendering namespace but found none. File content contains namespace declaration but parser failed to extract it.`);
      }
      
      // Verify specific expected symbols
      const expectedSymbols = [
        { name: 'BufferFactory', kind: 'class', expectedNamespace: 'PlanetGen::Rendering' },
        { name: 'CreateStandardUniformBuffer', kind: 'function', expectedNamespace: 'PlanetGen::Rendering' }
      ];
      
      for (const expected of expectedSymbols) {
        const found = symbols.find((s: any) => s.name === expected.name && s.kind === expected.kind);
        if (!found) {
          console.log(`[NAMESPACE_TEST] WARNING: Expected symbol ${expected.name} (${expected.kind}) not found`);
          continue;
        }
        
        if (found.namespace !== expected.expectedNamespace) {
          throw new Error(`Symbol ${expected.name}: expected namespace '${expected.expectedNamespace}', got '${found.namespace}'`);
        }
        
        console.log(`[NAMESPACE_TEST] ✓ Symbol ${expected.name} correctly has namespace ${found.namespace}`);
      }
