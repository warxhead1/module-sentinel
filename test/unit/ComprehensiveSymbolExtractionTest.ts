...
export class ComprehensiveSymbolExtractionTest {
  ...
  private async testPlanetBuilderMethodExtraction(): Promise<TestResult> {
    ...
    try {
      ...
      const expectedMethod: ExpectedSymbol = {
        ...
        qualifiedName:
          "PlanetGen::Generation::Factory::PlanetBuilder::WithTextureResolution",
        kind: "function",
  ...
  private async testAdvancedHeightGeneratorMethod(): Promise<TestResult> {
    ...
    try {
      ...
      const expectedMethod: ExpectedSymbol = {
        ...
        qualifiedName:
          "PlanetGen::Generation::AdvancedHeightGenerator::GenerateHeightSpherical",
        kind: "function",
        ...
        parameters: [
          ...
          {
            name: "coordinates",
            type: "const std::vector<std::pair<float, float>>&",
          },
        ],
      ...
      const method = dbSymbols.find(
        (s) =>
          ...
          (s.qualifiedName?.includes("AdvancedHeightGenerator") ||
            s.namespace?.includes("AdvancedHeightGenerator"))
      );
      ...
      if (!method) {
        ...
        dbSymbols.forEach((s) => {
          console.log(
            `  - ${s.kind} ${s.qualifiedName || s.name} at line ${s.line}`
          );
        });
      ...
      if (method.returnType !== expectedMethod.returnType) {
        errors.push(
          `Return type mismatch: expected "${expectedMethod.returnType}", got "${method.returnType}"`
        );
      }
      ...
      if (method.signature) {
        if (!method.signature.includes("HeightGenerationParameters")) {
          errors.push(
            "HeightGenerationParameters parameter type not found in signature"
          );
        }
        if (
          !method.signature.includes("std::vector") &&
          !method.signature.includes("vector")
        ) {
      ...
      if (errors.length > 0) {
        throw new Error(
          "Complex method extraction incomplete:\n" + errors.join("\n")
        );
      }
    ...
    try {
      ...
      for (const file of files) {
        ...
        const expectedNamespace = file.includes("PlanetBuilder.cpp")
          ? "PlanetGen::Generation::Factory"
          : "PlanetGen::Generation";

        const hasCorrectNamespace = namespaces.some(
          (ns) =>
        ...
        if (!hasCorrectNamespace) {
          ...
          throw new Error(
            `${expectedNamespace} namespace not found in ${file}`
          );
        }
        ...
        const withoutNamespace = classesAndFunctions.filter(
          (s) =>
            !s.namespace ||
            (!s.namespace.includes("PlanetGen") &&
              !s.qualifiedName?.includes("PlanetGen"))
        );

        if (withoutNamespace.length > 0) {
          console.log(
            `\n[WARNING] Symbols without proper namespace in ${path.basename(
              file
            )}:`
          );
          withoutNamespace.forEach((s) => {
            console.log(
              `  - ${s.kind} ${s.name} (namespace: ${s.namespace || "none"})`
            );
          });
        }
      }
  ...
  private async testParameterExtraction(): Promise<TestResult> {
    ...
    try {
      ...
      const methodsWithParams = dbSymbols.filter(
        (s) =>
      ...
      for (const check of paramChecks) {
        const method = methodsWithParams.find(
          (m: any) => m.name === check.method
        );
        if (method) {
          if (!method.signature?.includes(check.expectedParam)) {
            throw new Error(
              `Parameter type "${check.expectedParam}" not found in ${check.method} signature: ${method.signature}`
            );
          }
      ...
      for (const check of returnTypeChecks) {
        ...
        if (method) {
          if (!method.returnType?.includes(check.expectedReturn)) {
            errors.push(
              `${check.method}: expected return type containing "${check.expectedReturn}", got "${method.returnType}"`
            );
          } else {
  ...
  private async testClassMembershipExtraction(): Promise<TestResult> {
    ...
    try {
      ...
      const planetBuilderClass = dbSymbols.find(
        (s: any) => s.name === "PlanetBuilder" && s.kind === "class"
      );
      ...
      const planetBuilderMethods = dbSymbols.filter(
        (s: any) =>
          (s.kind === "function" || s.kind === "method") &&
          s.qualifiedName?.includes("PlanetBuilder::")
      );
      ...
      const missingClass = planetBuilderMethods.filter(
        (m: any) => !m.qualifiedName?.includes("PlanetBuilder")
      );
      ...
      if (missingClass.length > 0) {
        ...
        throw new Error(
          `${missingClass.length} methods not properly associated with PlanetBuilder class`
        );
      }
  ...
  private async testTemplateParameterExtraction(): Promise<TestResult> {
    ...
    try {
      ...
      const symbolsWithTemplates = dbSymbols.filter(
        (s: any) =>
          s.signature?.includes("<") ||
          s.returnType?.includes("<") ||
          (typeof s.semanticTags === "string"
            ? JSON.parse(s.semanticTags)
            : s.semanticTags
          )?.includes("template")
      );
      ...
      const hasVectorParam = dbSymbols.some(
        (s: any) =>
          s.signature?.includes("std::vector") ||
          s.signature?.includes("vector<")
      );

      const hasUniquePtr = dbSymbols.some(
        (s: any) =>
  ...
  private async testAdvancedDataTypeDetection(): Promise<TestResult> {
    ...
    try {
      ...
      const dataTypeChecks = {
        smart_pointers: allSymbols.filter(
          (s) =>
            s.signature?.includes("unique_ptr") ||
        ...
        stl_containers: allSymbols.filter(
          (s) =>
            s.signature?.includes("std::vector") ||
        ...
        const_references: allSymbols.filter(
          (s) => s.signature?.includes("const ") && s.signature?.includes("&")
        ),
        ...
        function_pointers: allSymbols.filter(
          (s) =>
            s.signature?.includes("(*)") ||
            s.signature?.includes("std::function")
        ),

        template_types: allSymbols.filter(
          (s) => s.signature?.includes("<") && s.signature?.includes(">")
        ),

        void_pointers: allSymbols.filter(
          (s) =>
      ...
      const totalAdvancedTypes = Object.values(dataTypeChecks).reduce(
      ...
      const complexSignatures = allSymbols.filter(
        (s) => s.signature && s.signature.length > 50
      );
  ...
  private async testComprehensiveReturnTypes(): Promise<TestResult> {
    ...
    try {
      ...
      const functionsWithReturnTypes = dbSymbols.filter(
        (s) => (s.kind === "function" || s.kind === "method") && s.returnType
      );
      ...
      const returnTypeCategories = {
        primitive: functionsWithReturnTypes.filter((f) =>
          [
            ...
            "uint32_t",
            "size_t",
          ].some((type) => f.returnType?.includes(type))
        ),
        ...
        pointers: functionsWithReturnTypes.filter(
          (f) => f.returnType?.includes("*") && !f.returnType?.includes("&")
        ),

        smart_pointers: functionsWithReturnTypes.filter(
          (f) =>
            f.returnType?.includes("unique_ptr") ||
            f.returnType?.includes("shared_ptr")
        ),

        stl_containers: functionsWithReturnTypes.filter(
          (f) =>
        ...
        custom_types: functionsWithReturnTypes.filter(
          (f) =>
            f.returnType &&
            ![
              "int",
      ...
      for (const check of expectedReturnTypes) {
        const method = functionsWithReturnTypes.find(
          (f: any) => f.name === check.method
        );
        if (method && method.returnType?.includes(check.expectedType)) {
          ...
          console.log(
            `  ⚠ ${check.method}: expected "${check.expectedType}", got "${method.returnType}"`
          );
        }
      }
  ...
  private async testComplexParameterTypes(): Promise<TestResult> {
    ...
    try {
      ...
      const functionsWithParams = dbSymbols.filter(
        (s) =>
          (s.kind === "function" || s.kind === "method") &&
          s.signature &&
          s.signature.includes("(") &&
      ...
      const parameterPatterns = {
        const_references: functionsWithParams.filter(
          (f) => f.signature?.includes("const ") && f.signature?.includes("&")
        ),

        nested_templates: functionsWithParams.filter(
          (f) =>
        ...
        function_parameters: functionsWithParams.filter(
          (f) =>
            f.signature?.includes("std::function<") ||
            f.signature?.includes("(*)")
        ),

        multiple_template_params: functionsWithParams.filter(
          (f) => (f.signature?.match(/</g) || []).length > 1
        ),
        ...
        perfect_forwarding: functionsWithParams.filter(
          (f) => f.signature?.includes("&&") && f.signature?.includes("T")
        ),
      };
      ...
      for (const check of complexSignatureChecks) {
        const method = functionsWithParams.find(
          (f: any) => f.name === check.method
        );
