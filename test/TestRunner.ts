...
export class TestRunner extends EventEmitter {
  ...
  constructor(options?: { forceRebuild?: boolean; testFilter?: string; maxFiles?: number }) {
    super();
  ...
  private async buildTestIndex(db: Database): Promise<void> {
    ...
    console.log(`✅ Indexed ${result.filesIndexed} files, found ${result.symbolsFound} symbols`);
  ...
  private async runTests(db: Database): Promise<TestResult[]> {
    ...
    const testClasses = [
      ...
      { name: 'ComprehensiveSymbolExtractionTest', class: ComprehensiveSymbolExtractionTest },
    ...
    for (const testDef of testClasses) {
      ...
      if (this.testFilter && !testDef.name.toLowerCase().includes(this.testFilter.toLowerCase())) {
        continue;
      }
      ...
      try {
        ...
        results.push(...testResults.map(r => ({
          ...r,
        ...
        for (const failedTest of failedTests) {
          console.log(`   ❌ ${failedTest.name}: ${failedTest.error?.message || 'Unknown error'}`);
        }
